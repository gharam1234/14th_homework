...
test.describe("PhoneNew 컴포넌트 UI", () => {
  ...
  test("페이지 제목이 렌더링되어야 함", async ({ page }) => {
    ...
    await expect(title).toHaveText("중고폰 판매하기");
  });

  test("모든 입력 필드가 렌더링되어야 함", async ({ page }) => {
    // 기기명 입력
    const phoneName = page.locator("[data-testid='input-phone-name']");
    await expect(phoneName).toBeVisible();
    await expect(phoneName).toHaveAttribute(
      "placeholder",
      "모델명을 입력해 주세요."
    );

    // 한줄 요약 입력
    const summary = page.locator("[data-testid='input-phone-summary']");
    ...
    // 판매 가격 입력
    const price = page.locator("[data-testid='input-phone-price']");
    ...
    // 태그 입력
    const tags = page.locator("[data-testid='input-phone-tags']");
  ...
  test("컨테이너가 올바른 크기를 가져야 함", async ({ page }) => {
    const container = page.locator("[data-testid='phone-new-container']");
