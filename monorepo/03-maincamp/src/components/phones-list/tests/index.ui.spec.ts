 ...
 */
test.describe("PhonesList Component", () => {
  ...
  test("컴포넌트가 렌더링되어야 함", async ({ page }) => {
    const component = page.locator("[data-testid='phones-list']");
  ...
  test("제목이 올바르게 표시되어야 함", async ({ page }) => {
    ...
    await expect(title).toContainText("여기에서만 만날 수 있는 중고폰");
  });
  ...
  test("중고폰 판매 등록 버튼이 클릭 가능해야 함", async ({ page }) => {
    const sellButton = page.locator("[data-testid='sell-button']");
